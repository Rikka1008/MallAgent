...
async def test_hybrid_retriever_falls_back_to_keyword_when_milvus_fails():
    ...
    retriever = HybridRetriever(client=BrokenClient(), vectorizer=FakeBgeM3Vectorizer(), keyword_retriever=FakeKeywordRetriever())
...
async def test_hybrid_retriever_uses_pool_when_client_is_not_injected(monkeypatch):
    ...
    async def mock_get_client():
        return client
    monkeypatch.setattr(
        "knowledge.retrieval.hybrid_retriever.MilvusClient.get_client", mock_get_client
    )
    retriever = HybridRetriever(dimension=4, vectorizer=FakeBgeM3Vectorizer())
